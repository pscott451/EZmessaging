...
internal class SmsDeliveredBroadcastReceiver: BroadcastReceiver() {
    ...
    fun buildPendingIntent(
        ...
        SmsSendCallbacks.deliveredCallbacks[callbackId] = onDelivered
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        if (context != null && intent != null) {
            ...
            SmsSendCallbacks.deliveredCallbacks[sendResultCallbackId]?.invoke(resultCode == Activity.RESULT_OK)
            SmsSendCallbacks.deliveredCallbacks.remove(sendResultCallbackId)
            context.unregisterReceiver(this)
        }
    }

    companion object {
        private const val ACTION_PREFIX = "com.scott.ezmessaging.manager.SMS_DELIVERED"
